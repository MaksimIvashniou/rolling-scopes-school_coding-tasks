...
function getNextFriday(date) {
  ...
  let increment = WEEK.friday - weekday;
  if (WEEK.friday <= weekday) increment += WEEK.length;

  date.setDate(day + increment);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (Date.parse(dateEnd) - Date.parse(dateStart)) / MS_PER_DAY + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  function findThursdayDate(targetDate) {
    const copy = new Date(targetDate);
    const thursdayDate = copy.getDate();

    const day = (copy.getDay() + 6) % WEEK.length;

    const increment = WEEK.Th - 1 - day;

    copy.setDate(thursdayDate + increment);

    return copy;
  }

  const endDate = findThursdayDate(date);

  const startDate = findThursdayDate(new Date(date.getFullYear(), 0, WEEK.Th));

  const weekDiff = (endDate - startDate) / WEEK.inMs;
...
function getNextFridayThe13th(date) {
  ...
  const incrementMonth = () => copy.setMonth(copy.getMonth() + 1);

  if (copy.getDate() >= SEARCH_CRITERIA.date) {
    incrementMonth();
  }

  copy.setDate(SEARCH_CRITERIA.date);

  while (copy.getDay() !== SEARCH_CRITERIA.weekDay) {
    incrementMonth();
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const [date, endDate] = Object.values(period).map((d) => {
  ...
  while (date <= endDate) {
    ...
    counter = (counter + increment) % countWorkDays;

    date.setDate(date.getDate() + increment + (counter ? 0 : countOffDays));
  }
