...
function formatTime(minutes, seconds) {
  return [minutes, seconds].map((v) => `${v}`.padStart(2, '0')).join(':');
}
...
function isPalindrome(str) {
  const origin = str.toLowerCase().match(/\w/g);
  return origin.toString() === origin.reverse().toString();
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/\w/gi, (char) => {
    ...
    const shiftDirection = char.toLowerCase() < 'n' ? SHIFT : -SHIFT;

    return String.fromCharCode(charCode + shiftDirection);
  });
}
...
function getCardId(value) {
  ...
  const [card, suit] = [value.slice(0, -1), value.slice(-1)];

  return SUITS.indexOf(suit) * CARDS.length + CARDS.indexOf(card);
}
